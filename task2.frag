...
void InitScene()
{
    ...
    // New rotating blue sphere.
    Sphere[2].center = vec3( -2.0 * cos(2.0 * iTime) + 1.0, 0.5, -2.0 * sin(iTime) );
    Sphere[2].radius = 0.4;
    Sphere[2].materialID = 3;
    
    // Another red sphere 
    Sphere[3].center = vec3( -3.0 * cos(iTime) - 1.0, 0.5, -3.0 * sin(iTime));
    Sphere[3].radius = 0.4;
    Sphere[3].materialID = 4;
    
    // non-reflective purple sphere.
    Sphere[4].center = vec3( 2.0 - 3.0 * cos(iTime), 0.5, -3.0 * sin(-2.0 * iTime));
    Sphere[4].radius = 0.6;
    Sphere[4].materialID = 5; // Non-reflective material
    ...
    // Light 0.
    Light[0].position = vec3( 2.0, 10.0, -4.0 );
    Light[0].I_a = vec3( 1.0, 0.8, 0.6  );
    Light[0].I_source = vec3( 0.5, 0.4, 0.35 );

    // Light 1.
    Light[1].position = vec3( -4.0 * cos(iTime) , 10.0, 0.0 );
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec3 cam_pos = vec3(0.5 + cos(angle) * 3.0, 2.5 + 2.0 * sin(angle / 2.0), 0.5 + 6.0 * sin(angle)); // 改变相机位置
